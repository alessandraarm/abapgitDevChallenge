CLASS z_challenge_week_4 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS z_challenge_week_4 IMPLEMENTATION.
  METHOD constructor.
    engine = NEW #( ).
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( interprete( 'WEST' )->get(  ) ).
    out->write( interprete( 'SOUTH' )->get(  ) ).
    out->write( interprete( 'OPEN BOX' )->get(  ) ).
    out->write( interprete( 'DROP RFC' )->get(  ) ).
